...
`@me` self-assign, `default` project default. ⚠️ `assign --assignee` resolves `@me`/`default`/**email** only — a raw **accountId silently UNassigns** (acli prints "unassigned" and clears it; verified). For a known accountId, use `bash ${CLAUDE_SKILL_DIR}/scripts/acli-assign.sh KEY ACCOUNT_ID` instead — it routes through `edit --from-json`, which resolves accountId correctly (verified 2026-07-15 against two different accounts; the bug is scoped to the `assign` subcommand only, not acli as a whole). Resolving an accountId from a privacy-hidden email in the first place is still MCP-only → [When acli can't](#when-acli-cant-fall-back-to-the-atlassian-mcp). `--generate-json` scaffolds any complex create/edit/link input. Per-flag tables → `REFERENCE.md`.

**Description format:** Jira `description`/comment `body` is ADF. Flags (`--description`/`--body`) wrap plain text into **one literal ADF paragraph — no markdown parsing.** `## heading`, `**bold**`, numbered/bulleted lists typed straight into the flag show up in Jira as those literal characters, not formatting — this is the #1 cause of a ticket or comment rendering as garbled plaintext. Use the flag only for a single unformatted sentence. Anything with headings/lists/bold/multiple sections needs a real ADF object: write Markdown and run `python3 ${CLAUDE_SKILL_DIR}/scripts/md2adf.py desc.md` (bare doc mode — omit `-s/-p/-t` for a comment/append body). Read it back with `${CLAUDE_SKILL_DIR}/scripts/adf2md.py`. `md2adf.py` parses GFM markdown tables (`| a | b |` + `|---|---|`) into real ADF `table` nodes (fixed 2026-07-15, `ISSUES.md` Issue 5; regression test: `scripts/test_md2adf.py`). It still only knows the Markdown subset in its own docstring — a description with node types outside that (e.g. `expand`, `panel`, an `extension`/macro) won't round-trip through a full-body replace. Count structural nodes in the current description before any full-body replace on an existing ticket that might carry one of those: `acli jira workitem view KEY --json | python3 ${CLAUDE_SKILL_DIR}/scripts/adf-node-diff.py -`; if `expand`/`panel`/`extension` count > 0, edit around it (`acli-edit.sh --replace-section` on an unaffected section) or patch the raw ADF directly instead of round-tripping through Markdown. After a full-body write, diff the intended patch against what's now live — `adf-node-diff.py intended.json <(acli jira workitem view KEY --json)` — a `byte-exact match: NO` on a write that should've been a pure passthrough is the signal something got silently dropped. Attachment differs by command: `workitem create/edit` → `--from-json`; `comment create` → `--body`/`--body-file` (auto-detect plain vs. ADF); `comment update` → dedicated `--body-adf FILE` flag. Confluence body is storage-format XHTML. Rules + GOOD/BAD → `REFERENCE.md` "Description & body formats" + `examples/`. **Content standard (what a description/comment should say, AC format, etc.) is not this skill's concern** — that's `jira-acli:jira-content` § `templates/` (Jira) or `jira-acli:confluence-content` § `templates/` (Confluence).
...
1. **Render what you're about to send.** For a `--from-json` create, round-trip it first: `python3 ${CLAUDE_SKILL_DIR}/scripts/md2adf.py desc.md -s "..." -p TP -t Bug > /tmp/wi.json && python3 ${CLAUDE_SKILL_DIR}/scripts/adf2md.py /tmp/wi.json` prints a readable card — `(new) <type>`, project, labels, and the full description. Eyeball it, *then* `acli jira workitem create --from-json /tmp/wi.json`. `${CLAUDE_SKILL_DIR}/scripts/acli-new.sh` (the one-step shortcut) does the same round-trip internally — call it once with `--dry-run` first to eyeball the same card, then call it again without the flag to actually create.
