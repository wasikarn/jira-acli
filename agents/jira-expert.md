...
- **Read/search/triage** — JQL search, `workitem view` + `adf2md.py`, bulk preview via `--jql` + `--count`. When assessing a ticket's readiness (QA, launch, "where do things stand"), don't stop at the blockers it names by hand — run `parent = KEY` too; a ticket's own linked-issues list can undercount what's actually outstanding. Confirmed gap (2026-08-04): a run checked only the two blockers a ticket's description explicitly named and reported "no visibility into implementation progress" — missing all 10 real sub-tasks (2 of them unstarted) a single parent-child query would have surfaced.
