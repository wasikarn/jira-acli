...
`.claude-plugin/plugin.json` + `marketplace.json` declare the plugin; `defaultEnabled: false` means installers must opt in via `settings.json`. Everything else lives under `skills/<name>/`, each with a required `SKILL.md` (frontmatter `name` + `description` triggers auto-invocation, `when_to_use` adds trigger phrases/exclusions — combined they're capped at 1,536 characters in the skill listing, per Claude Code's official frontmatter spec) plus `scripts/` and/or `references/` loaded on demand.
